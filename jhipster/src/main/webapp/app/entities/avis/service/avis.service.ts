...
type RestOf<T extends IAvis | NewAvis> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class AvisService {
  ...
  search(req: Search): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<RestAvis[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
      .pipe(map(res => this.convertResponseArrayFromServer(res)));
  }
  ...
  protected convertDateFromServer(restAvis: RestAvis): IAvis {
    return {
      ...restAvis,
      date: restAvis.date ? dayjs(restAvis.date) : undefined,
    };
  }
