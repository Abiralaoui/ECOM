...
import { EntityArrayResponseType, LigneCommandeService } from '../service/ligne-commande.service';
...
export class LigneCommandeComponent implements OnInit {
  ...
  trackId = (_index: number, item: ILigneCommande): number => this.ligneCommandeService.getLigneCommandeIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.ligneCommandes = this.refineData(dataFromBody);
  }

  protected refineData(data: ILigneCommande[]): ILigneCommande[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ILigneCommande[] | null): ILigneCommande[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean, currentSearch?: string): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.ligneCommandeService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.ligneCommandeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
