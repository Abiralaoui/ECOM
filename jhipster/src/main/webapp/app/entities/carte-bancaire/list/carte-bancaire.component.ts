...
import { EntityArrayResponseType, CarteBancaireService } from '../service/carte-bancaire.service';
...
export class CarteBancaireComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['nomPorteur'];
  ...
  trackId = (_index: number, item: ICarteBancaire): number => this.carteBancaireService.getCarteBancaireIdentifier(item);

  search(query: string): void {
    if (query && CarteBancaireComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (CarteBancaireComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.carteBancaires = this.refineData(dataFromBody);
  }

  protected refineData(data: ICarteBancaire[]): ICarteBancaire[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ICarteBancaire[] | null): ICarteBancaire[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean, currentSearch?: string): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.carteBancaireService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.carteBancaireService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
