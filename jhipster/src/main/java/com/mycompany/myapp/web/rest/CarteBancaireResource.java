...
public class CarteBancaireResource {
    ...
    /**
     * {@code SEARCH  /_search/carte-bancaires?query=:query} : search for the carteBancaire corresponding
     * to the query.
     *
    ...
    public List<CarteBancaireDTO> searchCarteBancaires(@RequestParam String query) {
        log.debug("REST request to search CarteBancaires for query {}", query);
        return carteBancaireService.search(query);
    }
}
