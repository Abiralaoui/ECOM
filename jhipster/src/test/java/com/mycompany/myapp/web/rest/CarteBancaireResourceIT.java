...
class CarteBancaireResourceIT {
    ...
    private static final Integer DEFAULT_NUM_CARTE = 1;
    private static final Integer UPDATED_NUM_CARTE = 2;
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/carte-bancaires";
    ...
    public void cleanupElasticSearchRepository() {
        ...
        assertThat(carteBancaireSearchRepository.count()).isEqualTo(0);
    }
    ...
    void createCarteBancaire() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(carteBancaireSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(carteBancaireSearchRepository.findAll());
    ...
    void putExistingCarteBancaire() throws Exception {
        ...
        carteBancaireSearchRepository.save(carteBancaire);
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(carteBancaireSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(carteBancaireSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<CarteBancaire> carteBancaireSearchList = IterableUtils.toList(carteBancaireSearchRepository.findAll());
                CarteBancaire testCarteBancaireSearch = carteBancaireSearchList.get(searchDatabaseSizeAfter - 1);
                assertThat(testCarteBancaireSearch.getNomPorteur()).isEqualTo(UPDATED_NOM_PORTEUR);
                assertThat(testCarteBancaireSearch.getNumCarte()).isEqualTo(UPDATED_NUM_CARTE);
                assertThat(testCarteBancaireSearch.getDateExpiration()).isEqualTo(UPDATED_DATE_EXPIRATION);
                assertThat(testCarteBancaireSearch.getCvv()).isEqualTo(UPDATED_CVV);
            });
    }
    ...
    @Test
    @Transactional
    void searchCarteBancaire() throws Exception {
        // Initialize the database
        carteBancaire = carteBancaireRepository.saveAndFlush(carteBancaire);
        carteBancaireSearchRepository.save(carteBancaire);

        // Search the carteBancaire
        restCarteBancaireMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + carteBancaire.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(carteBancaire.getId().intValue())))
            .andExpect(jsonPath("$.[*].nomPorteur").value(hasItem(DEFAULT_NOM_PORTEUR)))
